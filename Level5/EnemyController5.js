import Enemy from "../Level5/Enemy5.js";
import MovingDirection from "../Level5/MovingDirection5.js";
  ...
  collisionDetection() {
    this.enemyRows.forEach((enemyRow) => {
      enemyRow.forEach((enemy, enemyIndex) => {
        ...
        const enemyIdentifier = `enemy_${enemyIndex}`;
        if (!this.hitsByEnemy[enemyIdentifier]) {
          this.hitsByEnemy[enemyIdentifier] = 0;
        }
        //checks if there are any collisions between enemy and bullets
        if (this.playerBulletController.collideWith(enemy)) {
          this.hitsByEnemy[enemyIdentifier]++;
          console.log(this.hitsByEnemy[enemyIdentifier])
          if (this.hitsByEnemy[enemyIdentifier] === 40){
  ...
  updateVelocityAndDirection() {
    ...
    for (const enemyRow of this.enemyRows) {
      if (enemyRow.length === 0) {
        // If the row is empty (no enemies), continue to the next row without performing any operations
        continue;
      }
